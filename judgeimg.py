...
path="/opt/scorer/home/dev/SDK/roi/"
key="AhfffLL3f1BYUJMnBnV8HZFAuvFS3ztJ65ZQv9UPo5j"
def send2line_now():
    buffer='now.png saved'
    curl = pycurl.Curl()
    curl.setopt(pycurl.URL, 'https://notify-api.line.me/api/notify')
    curl.setopt(pycurl.HTTPHEADER, ['Authorization: Bearer ' + key])
    curl.setopt(pycurl.HTTPPOST, [('message', buffer.encode('utf-8')),('imageFile', (curl.FORM_FILE, path + 'on.png'))])
    curl.perform()
...
template1 = cv2.imread(path + 'on.png',1)
template2 = cv2.imread(path + 'off.png',1)
...
cv2.imwrite("res1.jpg",cv2.cvtColor(res1, cv2.COLOR_GRAY2BGR))
cv2.imwrite("result1.png",img1)
cv2.imwrite("template1.png",template1)
cv2.imwrite("res2.jpg",cv2.cvtColor(res2, cv2.COLOR_GRAY2BGR))
